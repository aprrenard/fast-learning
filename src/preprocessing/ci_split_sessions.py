...
for mouse_id in mice_id:
    ...
    if not os.path.exists(imaging_folder):
        continue
    sessions_list = os.listdir(imaging_folder)
    ...
    for isession in sessions_list:
        session_path = os.path.join(imaging_folder, isession)
        tif_paths = [os.path.join(session_path, itif) for itif in os.listdir(session_path)]
        nframes = 0
    ...
    for isession, session in enumerate(sessions_list):
    ...
    for itif, tif in enumerate(reg_tif_list):
        session = sessions_list[isession]
        ...
        # Case where you need to split a tif.
            ...
            next_session = sessions_list[isession+1]
