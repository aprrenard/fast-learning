...
# Color palettes.
reward_palette = sns.color_palette(['#d95f02', '#1b9e77'])
...
data = dfs.groupby(['mouse_id', 'lmi', 'reward_group', 'trial', 'cell_type'])['activity'].agg('mean').reset_index()
...
sns.relplot(data=data.loc[data.trial<100], x='trial', y='activity', col='lmi', row='reward_group', kind='line', palette=reward_palette, height=3, aspect=0.8)
...
sns.lineplot(data=data, x='trial', y='activity', hue='cell_type', palette=cell_types_palette,)

data = dfs.groupby(['mouse_id', 'lmi', 'reward_group', 'trial'])['activity'].agg('mean').reset_index()
data = data.loc[(data.reward_group=='R+') & (data.lmi=='positive')]
data = data.loc[data.trial<100]
