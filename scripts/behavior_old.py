...
def plot_single_mouse_performance(data,mouse_id,palette):
    """_summary_
    ...
    color_wh = palette[2] 
    if behavior_group == 'wh_non_rewarded':  # Green for whisker R+, red for R-.
        color_wh = palette[3]
    ...
    d = data.loc[(data.session_id.isin(sessions))]
    ...
    sns.lineplot(data=d, x='session_day', y='hr_a', color=palette[0], ax=axes[0],
                marker='o')
    if behavior_group in ['wh_rewarded', 'wh_non_rewarded']:
        sns.lineplot(data=d, x='session_day', y='hr_w', color=color_wh, ax=axes[0],
                    marker='o')

    # Plot performance to whisker day.
    d = data.loc[(data.mouse_id==mouse_id) & (data.session_day=='0')]
    ...
    d = d = data.loc[(data.mouse_id==mouse_id) & (data.session_day=='0')]
    ...
    axes[1].set_xticks(range(0,nmax_trials + nmax_trials%20,20))
...
SESSIONS_DB_PATH = 'D:\\z_LSENS\\Share\\Robin_Dard\\behavior_data\\behavior_sessions.xlsx'
DATA_FOLDER_PATH = 'D:\\z_LSENS\\Share\\Robin_Dard\\behavior_data'
PALETTE = ['#225ea8', '#00FFFF', '#238443', '#d51a1c', '#666666']
PALETTE = sns.color_palette(PALETTE)

# Read behavior results.
df_result = read_results(SESSIONS_DB_PATH, DATA_FOLDER_PATH)
...
def plot_average_across_days(data,mice,behavior_group,palette,nmax_trials=350,ax=None):
    ...
    color_wh = palette[2]
    if behavior_group=='wh_non_rewarded':
        color_wh = palette[3]
...
def plot_perf_across_blocks(data,mice,behavior_group,palette,nmax_blocks=None,ax=None):
...
BEHAVIOR_GROUP = 'wh_rewarded'
data = df_result.loc[(df_result.mouse_id.isin(mice)) & (df_result.behavior_group==BEHAVIOR_GROUP)]
plot_average_across_days(data, mice, BEHAVIOR_GROUP, PALETTE, ax=axes[0,0])
plot_perf_across_blocks(data, mice, BEHAVIOR_GROUP, PALETTE, ax=axes[0,1])

BEHAVIOR_GROUP = 'wh_non_rewarded'
data = df_result.loc[(df_result.mouse_id.isin(mice)) & (df_result.behavior_group==BEHAVIOR_GROUP)]
plot_average_across_days(data, mice, BEHAVIOR_GROUP, PALETTE, ax=axes[1,0])
plot_perf_across_blocks(data, mice, BEHAVIOR_GROUP, PALETTE, ax=axes[1,1])
sns.despine(fig=f)
pdf.savefig()
plt.close() 
...
data = df_result.loc[(df_result.mouse_id.isin(mice)) & (df_result.trial <=nmax_trials) & (df_result.behavior_group==behavior_group)]
data = data.groupby(['mouse_id','session_id','session_day'], as_index=False).agg(np.mean)
# Sort dataframe by days.
