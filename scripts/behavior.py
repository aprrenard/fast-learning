...
def plot_single_mouse_performance_across_days(df,mouse_id,color_palette):
    ...
    marker_width = 2
    block_size = 20
    behavior_group = df.loc[df.mouse_id==mouse_id, 'behavior_group'].iloc[0]
    color_wh = color_palette[2]
    if behavior_group == 'R-':  # green for whisker R+, red for R-.
...
def plot_single_session(df, session_id, palette):
    ...
    figsize = (15,6)
    block_size = 20
    behavior_group = df.loc[df.session_id==session_id, 'behavior_group'].iloc[0]
    color_wh = palette[2]
    if behavior_group == 'R-':  # green for whisker R+, red for R-.
        color_wh = palette[3]
    ...
    d = df.loc[df.session_id==session_id]
    ...
    if not d.hr_c.isna().all():
        sns.lineplot(data=d,x='trial',y='hr_c',color=palette[5],ax=ax,
                    marker='o')
    if not d.hr_a.isna().all():
        sns.lineplot(data=d,x='trial',y='hr_a',color=palette[0],ax=ax,
                    marker='o')
    if not d.hr_w.isna().all():
        sns.lineplot(data=d,x='trial',y='hr_w',color=color_wh,ax=ax,
                     marker='o')

    # Plot single trial raster plot.
    d = df.loc[df.session_id==session_id]
    if not d.hr_c.isna().all():
        ax.scatter(x=d.loc[d.outcome==0]['trial'],y=d.loc[d.outcome==0]['outcome_c']-0.1,
                        color=palette[4],marker=raster_marker, linewidths=marker_width)
        ax.scatter(x=d.loc[d.outcome==1]['trial'],y=d.loc[d.outcome==1]['outcome_c']-1.1,
                        color=palette[5],marker=raster_marker, linewidths=marker_width)

    if not d.hr_a.isna().all():
        ax.scatter(x=d.loc[d.outcome==0]['trial'],y=d.loc[d.outcome==0]['outcome_a']-0.15,
                        color=palette[1],marker=raster_marker, linewidths=marker_width)
        ax.scatter(x=d.loc[d.outcome==1]['trial'],y=d.loc[d.outcome==1]['outcome_a']-1.15,
                        color=palette[0],marker=raster_marker, linewidths=marker_width)

    if not d.hr_w.isna().all():
        ax.scatter(x=d.loc[d.outcome==0]['trial'],y=d.loc[d.outcome==0]['outcome_w']-0.2,
                        color=palette[3],marker=raster_marker, linewidths=marker_width)
        ax.scatter(x=d.loc[d.outcome==1]['trial'],y=d.loc[d.outcome==1]['outcome_w']-1.2,
                        color=palette[2],marker=raster_marker, linewidths=marker_width)

    nmax_trials = d.trial.max()
    ax.set_ylim([-0.2,1.05])
...
def plot_average_across_days(data,mice,behavior_group,palette,nmax_trials=300,ax=None):

    data = data.loc[(data.mouse_id.isin(mice)) & (data.trial <=nmax_trials) & (data.behavior_group==behavior_group)]
...
def plot_perf_across_blocks(data,mice,behavior_group,palette,nmax_trials=300,ax=None):
...
#     figsize = (18,6)
#     block_size = 20
#     behavior_group = data.loc[data.mouse_id==mouse_id, 'behavior_group'].iloc[0]
#     color_wh = palette[2]
#     if behavior_group == 'R-':  # green for whisker R+, red for R-.
#         color_wh = palette[3]
...
def read_results_excel(sessions_db_path, data_folder_path):
    ...
    for _, isession in sessions.iterrows():
        # mouse_id, session_folder, session_id, session_day, behavior_group, weight_initial, weight_before, batch_id = isession
        mouse_id, session_id, session_day, behavior_group, inactivation, inactivation_type = isession
        ...
        df['behavior_group'] = str(behavior_group)
...
SESSIONS_DB_PATH = "C:\\Users\\aprenard\\recherches\\fast-learning\\docs\\sessions_projection_neurons_short.xlsx"
# SESSIONS_DB_PATH = 'C:\\Users\\aprenard\\recherches\\fast-learning\\docs\\sessions_muscimol_GF.xlsx'
DATA_FOLDER_PATH = '\\\\sv-nas1.rcp.epfl.ch\\Petersen-Lab\\data'
# df_behav = read_results_excel(SESSIONS_DB_PATH, DATA_FOLDER_PATH)
if 'GF' in SESSIONS_DB_PATH:
    df_behav = read_results_GF(SESSIONS_DB_PATH, DATA_FOLDER_PATH)
else:
    df_behav = read_results_excel(SESSIONS_DB_PATH, DATA_FOLDER_PATH)
...
# Exclude early licks.
df_behav = df_behav.loc[df_behav.early_lick==0]
...
df_behav = df_behav.sort_values(['mouse_id','session_id','trial_number'])
df_behav['trial'] = df_behav.loc[(df_behav.early_lick==0)].groupby('session_id').cumcount()+1
# Add trial number for each stimulus.
df_behav['trial_w'] = df_behav.loc[(df_behav.early_lick==0) & (df_behav.is_whisker==1.0)]\
                                .groupby('session_id').cumcount()+1
df_behav['trial_a'] = df_behav.loc[(df_behav.early_lick==0) & (df_behav.is_auditory==1.0)]\
                                .groupby('session_id').cumcount()+1
df_behav['trial_c'] = df_behav.loc[(df_behav.early_lick==0) & (df_behav.is_stim==0.0)]\
                                .groupby('session_id').cumcount()+1

# Add performance outcome column for each stimulus.
df_behav['outcome_w'] = df_behav.loc[(df_behav.is_whisker==1.0)]['outcome']
df_behav['outcome_a'] = df_behav.loc[(df_behav.is_auditory==1.0)]['outcome']
df_behav['outcome_c'] = df_behav.loc[(df_behav.is_stim==0.0)]['outcome']

# Cumulative sum performance representation.
df_behav['cumsum_w'] = df_behav.loc[(df_behav.is_whisker==1.0)]['outcome_w']\
                                 .map({0:-1,1:1,np.nan:0})
df_behav['cumsum_w'] = df_behav.groupby('session_id')['cumsum_w'].transform(np.cumsum)
df_behav['cumsum_a'] = df_behav.loc[(df_behav.is_auditory==1.0)]['outcome_a']\
                                 .map({0:-1,1:1,np.nan:0})
df_behav['cumsum_a'] = df_behav.groupby('session_id')['cumsum_a'].transform(np.cumsum)
df_behav['cumsum_c'] = df_behav.loc[(df_behav.is_stim==0.0)]['outcome_c']\
                                 .map({0:-1,1:1,np.nan:0})
df_behav['cumsum_c'] = df_behav.groupby('session_id')['cumsum_c'].transform(np.cumsum)


# Add block index of n=BLOCK_SIZE trials and compute hit rate on them.
df_behav['block'] = df_behav.loc[df_behav.early_lick==0, 'trial'].transform(lambda x: x // BLOCK_SIZE)
# Compute hit rates. Use transform to propagate hit rate to all entries.
df_behav['hr_w'] = df_behav.groupby(['session_id', 'block'], as_index=False)['outcome_w']\
                             .transform(np.nanmean)
df_behav['hr_a'] = df_behav.groupby(['session_id', 'block'], as_index=False)['outcome_a']\
                             .transform(np.nanmean)
df_behav['hr_c'] = df_behav.groupby(['session_id', 'block'], as_index=False)['outcome_c']\
                             .transform(np.nanmean)

# Also store performance in new data frame with block as time steps for convenience.
cols = ['session_id','mouse_id','session_day','behavior_group','block']
df_block_perf = df_behav.loc[df_behav.early_lick==0, cols].drop_duplicates()
df_block_perf = df_block_perf.reset_index()
# Compute hit rates.
df_block_perf['hr_w'] = df_behav.groupby(['session_id', 'block'], as_index=False)\
                                 .agg(np.nanmean)['outcome_w']
df_block_perf['hr_a'] = df_behav.groupby(['session_id', 'block'], as_index=False)\
                                 .agg(np.nanmean)['outcome_a']
df_block_perf['hr_c'] = df_behav.groupby(['session_id', 'block'], as_index=False)\
...
# mice = df_behav.loc[(df_behav.batch_id==batch), 'mouse_id']
mice = df_behav.loc[(df_behav.inactivation_type=='execution'), 'mouse_id']
...
BEHAVIOR_GROUP = 'R+'
data = df_behav.loc[(df_behav.mouse_id.isin(mice)) & (df_behav.behavior_group==BEHAVIOR_GROUP)]
plot_average_across_days(data, mice, BEHAVIOR_GROUP, PALETTE, nmax_trials=NMAX_TRIAL, ax=axes[0,0])
plot_perf_across_blocks(data, mice, BEHAVIOR_GROUP, PALETTE, nmax_trials=NMAX_TRIAL, ax=axes[0,1])

BEHAVIOR_GROUP = 'R-'
data = df_behav.loc[(df_behav.mouse_id.isin(mice)) & (df_behav.behavior_group==BEHAVIOR_GROUP)]
plot_average_across_days(data, mice, BEHAVIOR_GROUP, PALETTE, nmax_trials=NMAX_TRIAL, ax=axes[1,0])
plot_perf_across_blocks(data, mice, BEHAVIOR_GROUP, PALETTE, nmax_trials=NMAX_TRIAL, ax=axes[1,1])
sns.despine(fig=f)
pdf.savefig()
plt.close()
...
sns.set_theme(context='talk', style='ticks', palette='deep', font='sans-serif', font_scale=0.8,
              rc={'font.sans-serif':'Arial'})
mice = df_behav.mouse_id.unique()
# mice = [m for m in mice if m not in ['AR111', 'AR112', 'AR113', 'AR114', 'AR081']]
pdf_path = 'C:\\Users\\aprenard\\recherches\\fast-learning\\results\\behavior'
pdf_fname = f'performance_across_days.pdf'
pdf = PdfPages(os.path.join(pdf_path,pdf_fname))

data = df_behav.loc[df_behav.trial<=nmax_trials]
...
for mouse_id in mice:
    ...
    for session_id in df_behav.loc[df_behav.mouse_id==mouse_id, 'session_id'].unique():
        print(session_id)
        # Call the plot_single_session function with the current session and mouse ID
        plot_single_session(df_behav, session_id, PALETTE)
...
# batches = [4]
# mice = df_behav.loc[(df_behav.batch_id.isin(batches)), 'mouse_id']
# mice = mice.drop_duplicates().to_list()
...
data = df_behav.loc[(df_behav.mouse_id.isin(mice)) & (df_behav.trial <=nmax_trials) & (df_behav.behavior_group==behavior_group)]
data = data.groupby(['mouse_id','session_id','session_day'], as_index=False).agg(np.mean)
# Sort dataframe by days.
...
df = df_behav.loc[df_behav.session_day=='0']
df = df.loc[df.is_whisker==1]

f = lambda x: x.reset_index(drop=True).idxmax()+1
fh = df.groupby(['mouse_id', 'behavior_group'], as_index=False)[['outcome_w']].agg(f)
fh.behavior_group = fh.behavior_group.astype("category")
fh.behavior_group = fh.behavior_group.cat.set_categories(['R+','R-'])
fh = fh.sort_values(['behavior_group'])

plt.figure(figsize=(4,4))
sns.swarmplot(data=fh, x='behavior_group', y='outcome_w', hue='behavior_group',palette=PALETTE[2:4],
                hue_order=['R+','R-'])
plt.ylim([0,16])
ax = plt.gca()
...
sns.lineplot(data=df, x='trial_w', y='cumsum_w', hue='behavior_group', palette=PALETTE[2:4],
...
plt.figure()
sns.lineplot(data=df_fh, x='trial_w', y='cumsum_w', hue='behavior_group', palette=PALETTE[2:4],
...
plt.figure(figsize=(10,6))
sns.lineplot(data=df_fh, x='trial_w', y='outcome_w', hue='behavior_group', palette=PALETTE[2:4],
             hue_order=['R+','R-'])
plt.xlim([0,20])
ax = plt.gca()
ax.set_xticks([0,5,10,15,20])
...
#     d_g1 = df_behav.loc[df_behav.session_id.isin(g1_sessions)]
#     d_g1 = d_g1.loc[d_g1.trial < nmax_trials]
#     d_g1 = d_g1.groupby(['mouse_id','session_id']).agg(np.nanmean)
#     d_g1['group'] = g1_label

#     d_g2 = df_behav.loc[df_behav.session_id.isin(g2_sessions)]
...
# # A2_R = df_behav.loc[(df_behav.reward_group==1) & (df_behav.batch_id==2) & (df_behav.session_day == 'A-2')]['session_id'].drop_duplicates()
# # A1_R = df_behav.loc[(df_behav.reward_group==1) & (df_behav.batch_id==2) & (df_behav.session_day == 'A-1')]['session_id'].drop_duplicates()
# # A2_NR = df_behav.loc[(df_behav.reward_group==0) & (df_behav.batch_id==2) & (df_behav.session_day == 'A-2')]['session_id'].drop_duplicates()
# # A1_NR = df_behav.loc[(df_behav.reward_group==0) & (df_behav.batch_id==2) & (df_behav.session_day == 'A-1')]['session_id'].drop_duplicates()
# # W1_R = df_behav.loc[(df_behav.reward_group==1) & (df_behav.batch_id==2) & (df_behav.session_day == 'W1')]['session_id'].drop_duplicates()
# # W1_NR = df_behav.loc[(df_behav.reward_group==0) & (df_behav.batch_id==2) & (df_behav.session_day == 'W1')]['session_id'].drop_duplicates()
# # P_ON1 = df_behav.loc[(df_behav.session_day == 'P_ON1')]['session_id'].drop_duplicates()
# # P_OFF1 = df_behav.loc[(df_behav.session_day == 'P_OFF1')]['session_id'].drop_duplicates()
# # P_ON2 = df_behav.loc[(df_behav.session_day == 'P_ON2')]['session_id'].drop_duplicates()


# # session_lists = [A2_R, A1_R, W1_R]
# # labels = ['D-2','D-1','D0']
# # plot_average_perf(df_behav, session_lists, labels, True,300)

# # session_lists = [A2_NR, A1_NR, W1_NR]
# # labels = ['D-2','D-1','D0']
# # plot_average_perf(df_behav, session_lists, labels, False,300)

# # session_lists = [P_ON1,P_OFF1,P_ON2]
# # labels = ['ON pre','OFF','ON post']
# # plot_average_perf(df_behav, session_lists, labels, True,300)

# # perf_stats(df_behav,P_ON1,P_OFF1,'P_ON','P_OFF',300,test='wilcoxon',alternative='greater')
# # perf_stats(df_behav,P_ON2,P_OFF1,'P_ON','P_OFF',300,test='wilcoxon',alternative='greater')

# # perf_stats(df_behav,W1,A1,'W1','A1',250,test='wilcoxon',alternative='greater')

# # perf_stats(df_behav,W1,CNO1_S1,'W1','CNO1_S1',250,test='mann_whitney',alternative='greater')
# # perf_stats(df_behav,W1,CNO1_S2p_S1,'W1','CNO1_S2',250,test='mann_whitney',alternative='greater')

# # perf_stats(df_behav,P_ON2,P_OFF1,'SAL2','CNO1',250,test='mann_whitney',alternative='greater')
# # perf_stats(df_behav,W3,CNO1_S2p_S1,'SAL2','CNO1',250,test='mann_whitney',alternative='greater')
...
# # plot_average_perf(df_behav, session_lists, labels, 100)
